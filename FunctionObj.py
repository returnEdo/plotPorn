...
def linspace(a: float, b: float, n: int) -> typing.List[float]:
	...
	return [step * i + a for i in range(n + 1)]




def computePoints(fun: typing.Callable([float], float),
				  leftX: float,
				  rightX: float,
				  n: int):
	...
	return X, [fun(x) for x in X]
	
	


class Function(Object.Object):
	...
	def __init__(self, fun: typing.Callable([float], float)):
	...
	def __str__(self) -> str: 	return 'Function'
